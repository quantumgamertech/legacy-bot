        ...
        class Database:
            ...
            def _setup(self):
                ...
                if not self._column_exists("guild_settings", "vote_reward_role_id"):
                    with self.conn:
                        self.conn.execute("ALTER TABLE guild_settings ADD COLUMN vote_reward_role_id INTEGER")
            ...
            def set_vote_reward_role(self, guild_id: int, role_id: int | None):
                self.ensure_guild(guild_id)
                with self.conn:
                    self.conn.execute(
        ...
        def format_dt_safe(value: str | None, style: str = "R") -> str:
            dt = iso_to_dt(value)
            if dt is None:
                return "Never"
        ...
        def build_vote_status_embed(user: discord.abc.User, guild: discord.Guild | None = None) -> discord.Embed:
            ...
            embed.add_field(
                name="Premium Until",
                value=format_dt_safe(data.get("premium_until"), "F") if data.get("premium_until") else "Not active",
                inline=True,
            )
            embed.add_field(
                name="Time Remaining",
            ...
            embed.add_field(name="Vote Link", value=f"[Vote on Top.gg]({get_topgg_vote_url()})", inline=False)
            return embed
        ...
        def calculate_next_vote_streak(old_last_vote_at: str | None, old_streak: int) -> int:
        ...
        async def process_topgg_vote(user_id: int, payload: dict, source: str = "topgg") -> dict:
            ...
            new_streak = calculate_next_vote_streak(current.get("last_vote_at"), int(current.get("streak") or 0))
        ...
        web_site: web.TCPSite | None = None
        ...
        async def start_web_server():
            ...
            web_runner = runner
            web_site = site
            log.info("Webhook server started on %s:%s route=%s", TOPGG_WEB_HOST, TOPGG_WEB_PORT, TOPGG_WEB_ROUTE)
        ...
        async def votestatus_command(ctx: commands.Context, member: discord.Member | None = None):
        ...
        @admin_or_manage_guild()
        async def setvoterole_command(ctx: commands.Context, role: discord.Role | None = None):
            if role is None:
        ...
        async def votestatus_slash(interaction: discord.Interaction, member: discord.Member | None = None):
        ...
        async def testvote_command(ctx: commands.Context, user_id: int):
            payload = {"user": str(user_id), "type": "test", "isWeekend": False, "manual": True}
            ...
            embed.add_field(name="Premium Until", value=format_dt_safe(result["premium_until"], "F"), inline=False)
            await ctx.send(embed=embed)
        ...
        async def voteadmin_command(ctx: commands.Context):
            ...
            if events:
                ...
                embed.add_field(name="Recent Vote Events", value="\n".join(event_lines), inline=False)
        ...
        async def servers_command(ctx: commands.Context):
            ...
            for guild in sorted_guilds[:20]:
                ...
                line = f"`{guild.id}` • **{guild.name}** • {guild.member_count or 0} members{premium_tag}{report_tag}{alerts_tag}{vote_role_tag}"
