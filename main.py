...
# Paste your real bot invite link here later
BOT_INVITE_URL = "PASTE_YOUR_BOT_INVITE_LINK_HERE"
...
def build_about_embed():
    embed = discord.Embed(
        title="✨ About Anniversary Bot",
        description="Anniversary Bot helps servers celebrate member milestones with announcements, progression, and milestone rewards.\n\u200b",
        color=EMBED_COLOR,
        timestamp=datetime.now(timezone.utc)
    )
    ...
    embed.add_field(
        name="What It Does",
        value=(
            "\n• Posts anniversary announcements automatically\n"
            "• Tracks milestone progression with `!mystats`\n"
...
def build_invite_embed():
    embed = discord.Embed(
        title="🔗 Invite Anniversary Bot",
        description="Use the link below to add Anniversary Bot to another server.\n\u200b",
        color=EMBED_COLOR,
        timestamp=datetime.now(timezone.utc)
    )

    if not BOT_INVITE_URL or BOT_INVITE_URL == "https://discord.com/oauth2/authorize?client_id=1483943578148405279&permissions=268520448&integration_type=0&scope=bot+applications.commands":
        invite_value = (
            "Invite link not set yet.\n"
            "Add your OAuth invite URL in `BOT_INVITE_URL` inside `main.py`."
        )
    else:
        invite_value = BOT_INVITE_URL

    embed.add_field(name="Invite Link", value=invite_value, inline=False)
    embed.add_field(
        name="After Inviting",
        value=(
            "\n1. Run `!setup`\n"
