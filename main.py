...
def generate_growth_dashboard_chart(guild: discord.Guild, days: int = 7) -> io.BytesIO:
    ...
    ax.plot(labels, cumulative, marker="o", linewidth=2.2, label="Cumulative Net")
    ax.bar(labels, daily_net, alpha=0.35, label="Daily Net")
    ax.set_title(f"{guild.name} • Premium Growth Dashboard ({days} Days)")
    ax.set_xlabel("Date")
...
def build_growth_dashboard_embed(guild: discord.Guild, days: int = 7) -> discord.Embed:
    ...
    if best_day is None:
        ...
        best_day_text = (
            ...
            f"(+{best_day['joins']} / -{best_day['leaves']})"
        )
    ...
    embed = build_main_embed(
        ...
        f"Detailed analytics for **{guild.name}** over the last **{days}** days.",
        discord.Color.gold(),
    )
    ...
    embed.add_field(name="Avg Daily Net", value=f"{summary['avg_daily_net']:+.2f}", inline=True)

    embed.add_field(
        name=f"{days}-Day Totals",
        value=(
    ...
    embed.add_field(name="Best Day", value=best_day_text, inline=False)
    embed.add_field(
...
async def dashboard_slash(
    ...
    await interaction.followup.send(embed=embed, file=dashboard_file)
