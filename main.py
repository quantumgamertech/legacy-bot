...
def get_guild_milestones(guild_id):
    conn = get_db_connection()
    cur = conn.cursor()

    cur.execute("""
        SELECT milestone_year, role_name
    ...
    return {int(year): role_name for year, role_name in rows}


def set_milestone_for_guild(guild_id, milestone_year, role_name):
    conn = get_db_connection()
    cur = conn.cursor()

    cur.execute("""
        INSERT INTO guild_milestones (guild_id, milestone_year, role_name)
        VALUES (%s, %s, %s)
        ON CONFLICT (guild_id, milestone_year)
        DO UPDATE SET role_name = EXCLUDED.role_name
    """, (str(guild_id), int(milestone_year), role_name))
...
def reset_milestones_for_guild(guild_id):
    conn = get_db_connection()
    cur = conn.cursor()

    cur.execute("""
        DELETE FROM guild_milestones
        WHERE guild_id = %s
    """, (str(guild_id),))
...
def get_milestone_role_name(years, guild_milestones):
    eligible_years = [milestone for milestone in guild_milestones if years >= milestone]
    if not eligible_years:
        return None
    highest = max(eligible_years)
    return guild_milestones[highest]


def get_next_milestone(years, guild_milestones):
    future_milestones = sorted([milestone for milestone in guild_milestones if milestone > years])
    if not future_milestones:
        return None
    next_year = future_milestones[0]
    return next_year, guild_milestones[next_year]


def get_missing_milestone_roles(guild, guild_milestones):
    missing = []
    for _, role_name in sorted(guild_milestones.items()):
        if discord.utils.get(guild.roles, name=role_name) is None:
            missing.append(role_name)
    return missing


def get_missing_milestone_years(guild_milestones):
    return [year for year in DEFAULT_MILESTONE_YEARS if year not in guild_milestones]


def get_manageability_issues(guild, guild_milestones):
    issues = []
...
async def assign_highest_milestone_role(member, guild, years, guild_milestones):
    target_role_name = get_milestone_role_name(years, guild_milestones)
    ...
    milestone_role_names = list(guild_milestones.values())
...
def build_help_embed(is_server_premium):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`!setmilestone` — Set a custom milestone role\n"
            "`!resetmilestones` — Clear all milestone roles\n"
    ...
    embed.add_field(
        ...
        value=(
            ...
            "2. Create the milestone roles you want to use\n"
            "3. Run `!setmilestone 1 <role>` and so on\n"
            "4. Move the bot role above milestone roles\n"
            "5. Upgrade to premium for custom messages and role rewards"
        ),
        inline=False
    )
    ...
    embed.add_field(
        ...
        value=(
            "\nUse:\n"
            "`!setmilestone 1 Rookie`\n"
            "`!setmilestone 2 Veteran`\n"
            "`!setmilestone 3 Legend`"
        ),
        inline=False
    )
...
def build_setup_embed(guild, is_server_premium, guild_milestones):
    missing_years = get_missing_milestone_years(guild_milestones)
    missing_roles = get_missing_milestone_roles(guild, guild_milestones) if guild_milestones else []
    manageability_issues = get_manageability_issues(guild, guild_milestones) if guild_milestones else []
    ...
    embed.add_field(
        name="Step 2",
        value="Create the milestone roles you want for your server.",
        inline=False
    )
    ...
    embed.add_field(
        name="Step 3",
        value=(
            "Set milestone roles with commands like:\n"
            "`!setmilestone 1 Rookie`\n"
            "`!setmilestone 2 Veteran`\n"
            "`!setmilestone 3 Legend`"
        ),
    ...
    if missing_years:
        embed.add_field(
            name="⚠️ Missing Milestone Setup",
            value="\n".join([f"• No role set for {format_years(year)}" for year in missing_years]),
            inline=False
        )
    ...
    if not missing_years and not missing_roles and not manageability_issues:
        embed.add_field(
            name="✅ Setup Check",
...
def build_milestones_embed(guild_milestones):
    embed = discord.Embed(
        title="🏆 Legacy Milestones",
        description="Here’s the milestone ladder for this server.\n\u200b",
        color=EMBED_COLOR,
        timestamp=datetime.now(timezone.utc)
    )
    ...
    if guild_milestones:
        milestone_lines = []
        for years, role_name in sorted(guild_milestones.items()):
        ...
        embed.add_field(
            name="Milestone Ladder",
            value="No milestone roles configured yet.\nUse `!setmilestone <years> <role name>`.",
            inline=False
        )
...
def build_serverstatus_embed(guild, is_server_premium_flag, settings, guild_milestones):
    missing_years = get_missing_milestone_years(guild_milestones)
    missing_roles = get_missing_milestone_roles(guild, guild_milestones) if guild_milestones else []
    manageability_issues = get_manageability_issues(guild, guild_milestones) if guild_milestones else []
    ...
    milestone_value = "Not configured"
    if guild_milestones:
        milestone_value = f"{len(guild_milestones)} milestone(s) set"
    ...
    embed.add_field(name="Milestones", value=milestone_value, inline=False)

    if missing_years:
        embed.add_field(
            name="Missing Milestone Setup",
            value="\n".join([f"• No role set for {format_years(year)}" for year in missing_years]),
            inline=False
        )
    else:
        embed.add_field(
            ...
            value="✅ Core milestone years configured",
            inline=False
        )
    ...
    if missing_roles:
        ...
        embed.add_field(
            name="Missing Roles",
            value="✅ All configured milestone roles found",
            inline=False
        )
...
def build_invite_embed():
    ...
    embed.add_field(
        ...
        value=(
            "\n1. Run `!setup`\n"
            "2. Run `!setchannel`\n"
            "3. Create the roles you want to use\n"
            "4. Set them with `!setmilestone`\n"
...
async def check_anniversaries():
    ...
    for guild in client.guilds:
        ...
        guild_milestones = get_guild_milestones(guild.id)
        ...
        for member in guild.members:
            ...
            if joined.month == now.month and joined.day == now.day:
                ...
                if years >= 1:
                    ...
                    if is_premium(guild.id) and guild_milestones:
                        granted_role_name = await assign_highest_milestone_role(member, guild, years, guild_milestones)
...
async def on_message(message):
    ...
    if message.content == "!test":
        ...
        guild_milestones = get_guild_milestones(message.guild.id)
        embed = build_setup_embed(message.guild, is_premium(message.guild.id), guild_milestones)
        await message.channel.send(embed=embed)
        ...
        guild_milestones = get_guild_milestones(message.guild.id)
        embed = build_milestones_embed(guild_milestones)
        await message.channel.send(embed=embed)
        ...
        if is_premium(message.guild.id) and guild_milestones:
            highest_preview_year = max(guild_milestones.keys())
            role_name = get_milestone_role_name(highest_preview_year, guild_milestones)
        ...
        guild_milestones = get_guild_milestones(message.guild.id)
        current_role_name = get_milestone_role_name(years, guild_milestones) if guild_milestones else None
        next_milestone = get_next_milestone(years, guild_milestones) if guild_milestones else None
...
Contact the server owner to get access."""
    ...
    elif message.content.startswith("!setmilestone "):
        if not message.author.guild_permissions.administrator:
            await message.channel.send("❌ You must be an admin.")
            return
        ...
        if len(parts) < 3:
            await message.channel.send("❌ Usage: `!setmilestone 1 Rookie`")
            return

        try:
            milestone_year = int(parts[1])
        except ValueError:
            await message.channel.send("❌ Milestone year must be a number.")
            return

        role_name = parts[2].strip()
        if not role_name:
            await message.channel.send("❌ Role name cannot be empty.")
            return
        ...
        try:
            set_milestone_for_guild(message.guild.id, milestone_year, role_name)
            await message.channel.send(f"✅ Milestone set: **{format_years(milestone_year)}** → **{role_name}**")
        except Exception as e:
            await message.channel.send(f"❌ Failed to save milestone: {e}")

    elif message.content == "!resetmilestones":
        if not message.author.guild_permissions.administrator:
            await message.channel.send("❌ You must be an admin.")
            return

        try:
            reset_milestones_for_guild(message.guild.id)
            await message.channel.send("✅ All milestone role settings cleared.")
        except Exception as e:
            await message.channel.send(f"❌ Failed to reset milestones: {e}")
...
Contact the server owner to get access."""
        ...
        guild_milestones = get_guild_milestones(message.guild.id)
        if not guild_milestones:
            await message.channel.send("❌ No milestone roles configured yet. Use `!setmilestone` first.")
            return

        missing_roles = get_missing_milestone_roles(message.guild, guild_milestones)
        manageability_issues = get_manageability_issues(message.guild, guild_milestones)
        ...
        highest_year = max(guild_milestones.keys())
        granted_role_name = await assign_highest_milestone_role(message.author, message.guild, highest_year, guild_milestones)
...
💎 Custom messages and custom milestone role rewards are premium features.
